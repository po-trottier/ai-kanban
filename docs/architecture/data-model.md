...
| resume_alerted_at                                    | TEXT NULL                  | claimed in the same transaction that selects the overdue card, BEFORE the DM is attempted — at-most-once per episode: a delivery failure does not re-fire; cleared on lane exit (changing the date means leaving and re-entering the lane with fresh values; no in-place edit in v1) |
