...
| resume_alerted_at                                    | TEXT NULL                  | set when the overdue DM fires; cleared on lane exit (one alert per episode — changing the date means leaving and re-entering the lane with fresh values; no in-place edit in v1)                                                                                                   |
