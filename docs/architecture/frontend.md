...
| `settings/`      | Tabbed settings page: a **Preferences** tab (timezone + theme) every role can open, plus the manage\*-gated admin tabs (users, board columns — add/rename/reorder/WIP/delete via `LanesAdmin`, permission policy editor, location tree, service tokens) shown only when the caller's role grants each `manage*` permission — there is no admins-only wall                                                                                                                                                            |
