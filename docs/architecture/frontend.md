...
| `settings/`      | Tabbed settings page: a **Preferences** tab (timezone + theme + About with the running server version) every role can open, plus the manage\*-gated admin tabs (users; board columns — add/rename/reorder/WIP/delete via `LanesAdmin`, which also hosts the workflow-transitions matrix + enforcement toggle via `TransitionsEditor` beneath the column list; the Permissions tab's role/permission editor via `PolicyEditorForm`; the **Hours** tab's business-hours editor via `WorkingHoursForm` — both `managePolicy`-gated and PUTting the same policy document; location tree; service tokens) shown only when the caller's role grants each `manage*` permission — there is no admins-only wall |
