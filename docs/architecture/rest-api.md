...
| GET /users                    | any           | active users (id, name, role) for pickers; `email` is included only for actors granted `manageUsers` (the admin users table) — gated on the permission, not a hardcoded role, so a custom admin role sees it too                                                                                                                                                                                                                              |
...
| GET /cards/:id                  | any                    | full detail: card + tags + location + attachment metadata. `:id` accepts the uuid OR the human ticket **number** (the web deep-links `/cards/1`); a numeric id resolves to the card's uuid (unknown number → 404). Every other `/cards/:id/*` route stays uuid-only — the SPA resolves the number to the uuid before calling them                                                                 |
...
| POST /service-tokens, GET /service-tokens, DELETE /service-tokens/:id | `manageTokens` | MCP credentials (403 `permission:manageTokens`); each token has a human `name` (shown in history as "<name> on behalf of <minter>"); raw token returned once on create; `role` is a role key validated against the active policy (unknown → 400) |
