...
| Integration | `*.integration.test.ts` (Vitest, forks pool) | the **real Fastify app** via `app.inject()` + a **real temp SQLite file** per test file with real migrations; real Bolt app via TestReceiver; external HTTP (Slack Web API, the summarizer's OpenAI-compatible endpoint) served by **local fixture HTTP servers** | < 1 s each   |
...
- Recorded Slack and OpenAI-compatible LLM JSON fixtures (`slack-payloads.ts`,
  `llm-responses.ts`) live in `packages/server/test/fixtures/` and are checked in; they are real
  captured payload shapes, trimmed and anonymized.
