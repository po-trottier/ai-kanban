...
/** Providers for a hook under test: injected fake API + the real QueryClient. */
...
describe('useMoveCard', () => {
  it('invalidates the moved card history so an open History tab updates live', async () => {
    ...
    // refetch (#88); before the fix the move touched only the board key. Seed the
    // events query so its invalidation is observable on the real client state.
    ...
    const eventsKey = queryKeys.events(String(card.id))
    ...
    // Assert — the events query for the moved card is now stale (will refetch).
