...
describe.each(['sqlite', 'postgres'])('OAuth on %s', (engine) => {
  let t: TestApp
  let cookie: string
  ...
  async function obtainCode(
    ...
    // Submit the fields the browser actually receives, including its CSRF token.
    const fields = Object.fromEntries(
      [...consent.body.matchAll(/<input type="hidden" name="([^"]+)" value="([^"]*)" \/>/g)].map(
        (match) => [match[1], match[2]],
      ),
    ) as Record<string, string>
    const approve = await postForm('/oauth/authorize', { ...fields, decision: 'approve' })
  ...
  beforeAll(async () => {
    t =
      engine === 'postgres'
        ? await createTestApp({
            dataLayer: await createPgDataLayerFrom(await openPgliteConnection()),
          })
        : await createTestApp()
  ...
  describe('OAuth authorization-server routes', () => {
    it.each([{ mustChangePassword: true }, { isActive: false }])(
      'rejects already-issued credentials when the operator becomes unavailable: %j',
      async (flags) => {
        ...
        try {
          ...
          const tokens = await freshTokens(clientId)
          const { verifier, challenge } = pkcePair()
          const { code } = await obtainCode(clientId, challenge)
          // Simulates a pre-upgrade grant whose account is now restricted.
          await t.wired.deps.uow.run((tx) => tx.userAccounts.update({ ...operator.user, ...flags }))
          ...
          expect((await refresh(clientId, tokens.refresh_token)).statusCode).toBe(400)
          expect((await codeGrant(clientId, code, verifier)).statusCode).toBe(400)
        } finally {
    ...
    it('blocks consent for an account that must change its temporary password', async () => {
      const { user, password } = await t.createUser('user', { mustChangePassword: true })
      const tempCookie = await t.login(user.email, password)
      const clientId = await registerClient()
      ...
      const fields = {
        ...
        code_challenge: challenge,
        code_challenge_method: 'S256',
        csrf: consentCsrfToken(tempCookie),
        decision: 'approve',
      }
      for (const method of ['GET', 'POST'] as const) {
        const response = await t.app.inject({
          ...
          url:
            method === 'GET'
              ? `/oauth/authorize?${new URLSearchParams(fields).toString()}`
              : '/oauth/authorize',
          ...(method === 'POST' ? form(fields) : {}),
          headers: {
            cookie: `sid=${tempCookie}`,
            ...(method === 'POST' ? form(fields).headers : {}),
          },
        })
    ...
    it.each(['change-password', 'reset-password', 'deactivate', 'role-change'])(
      'revokes OAuth access, refresh, and pending codes on %s',
      async (action) => {
        const operator = await t.asRole('user')
        const admin = await t.asRole('admin')
        const originalCookie = cookie
        ...
        try {
          ...
          const { verifier, challenge } = pkcePair()
          const { code } = await obtainCode(clientId, challenge)
          let changed: LightMyRequestResponse
          if (action === 'change-password') {
            changed = await t.request(operator.cookie, {
              method: 'POST',
            ...
            const payload =
              action === 'reset-password'
                ...
                : action === 'deactivate'
                  ? { isActive: false }
                  : { role: 'admin' }
            changed = await t.request(admin.cookie, {
              method: 'PATCH',
              url: `/api/v1/users/${operator.user.id}`,
              payload,
            })
          }
          expect(changed.statusCode).toBe(action === 'change-password' ? 204 : 200)
          const access = await t.app.inject({
            method: 'POST',
            url: '/mcp',
            headers: {
    ...
    it('revokes a refresh token (RFC 7009), always returning 200', async () => {
      const clientId = await registerClient()
      const { refresh_token, access_token } = await freshTokens(clientId)
      const otherClient = await registerClient()
      const otherTokens = await freshTokens(otherClient)
      ...
      expect(access.statusCode).toBe(401)
      expect((await refresh(otherClient, otherTokens.refresh_token)).statusCode).toBe(200)

      // The revoked token can no longer refresh.
      expect((await refresh(clientId, refresh_token)).statusCode).toBe(400)

      // Replaying a revoked credential must not revoke a later authorization.
      const reauthorized = await freshTokens(clientId)
      await postForm('/oauth/revoke', { token: refresh_token, client_id: clientId }, false)
      const live = await t.wired.deps.uow.read((tx) =>
        tx.oauthAccessTokens.findByHash(
          createHash('sha256').update(reauthorized.access_token).digest('hex'),
        ),
      )
