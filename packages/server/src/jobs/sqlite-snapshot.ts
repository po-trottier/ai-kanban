...
/**
 * Nightly online-backup snapshot (docs/architecture/deployment.md
 * #database-operations): a live WAL database must never be file-copied, so
 * the nightly backup is a self-contained copy under SNAPSHOT_DIR, dated by
 * the job clock, with the newest 7 retained. Idempotent: a restart on the
 * same UTC day sees the file already exists and only prunes — sound because
 * the store publishes atomically (a dated file is always a COMPLETED
...
export interface SnapshotStore {
  /**
   * Back the live database up into the given filename under SNAPSHOT_DIR
   * (must not pre-exist). Must publish atomically: a crash mid-write must
   * never leave a partial file under the final name — the job's same-day
   * idempotency check treats an existing dated file as a completed snapshot
   * and would neither retry nor prune it for SNAPSHOT_RETAIN_COUNT days.
   */
  backupInto(filename: string): Promise<void>
...
export async function runSqliteSnapshot(
  ...
  if (!existing.includes(filename)) {
    await deps.snapshots.backupInto(filename)
    created = filename
